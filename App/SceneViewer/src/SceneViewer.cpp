...
const char* s_pAssetDirectory = LUZ_ASSET_DIRECTORY;
...
//    Platform::CreateConsole();
    ...
    // Shaders
    {
        ...
        {         
            m_vsScene = Graphics::CreateVertexShader(std::string(SCENEVIEWER_SHADER_DIRECTORY).append("scene_model_vs.hlsl").c_str());
            m_psScene = Graphics::CreatePixelShader(std::string(SCENEVIEWER_SHADER_DIRECTORY).append("scene_model_ps.hlsl").c_str());

            m_vsFullScreen = Graphics::CreateVertexShader(std::string(SCENEVIEWER_SHADER_DIRECTORY).append("FS_Tri_VS.hlsl").c_str());
            m_psFullScreen = Graphics::CreatePixelShader(std::string(SCENEVIEWER_SHADER_DIRECTORY).append("FS_Tri_PS.hlsl").c_str());

            m_vsCubeMap = Graphics::CreateVertexShader(std::string(SCENEVIEWER_SHADER_DIRECTORY).append("cube_map_vs.hlsl").c_str());
            m_psCubeMap = Graphics::CreatePixelShader(std::string(SCENEVIEWER_SHADER_DIRECTORY).append("cube_map_ps.hlsl").c_str());

            m_psRadianceMap = Graphics::CreatePixelShader(std::string(SCENEVIEWER_SHADER_DIRECTORY).append("radiance_map_ps.hlsl").c_str());
            m_psIrradianceMap = Graphics::CreatePixelShader(std::string(SCENEVIEWER_SHADER_DIRECTORY).append("irradiance_map_ps.hlsl").c_str());
            m_psLUT = Graphics::CreatePixelShader(std::string(SCENEVIEWER_SHADER_DIRECTORY).append("brdf_integration_ps.hlsl").c_str());
        }
        else
        {
    ...
    // Textures
    {
        ...
        // Sky box
        {
            char pCubeMapPath[MAX_PATH];
            strcpy_s(pCubeMapPath, s_pAssetDirectory);
            strcat_s(pCubeMapPath, "satara_night.dds");

            Graphics::TextureFileDesc fd;
            fd.Filename = pCubeMapPath;
            fd.GenMips = false;
    ...
    params.LightingMode = SceneResource::SHADING_MODE_BECKMANN;
